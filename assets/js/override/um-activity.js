...
jQuery(document).ready(function() {
	...
	jQuery(document).on('submit', '.um-activity-publish', function(e){
		...
		if ( this_form.find('textarea').val().trim().length == 0 && this_form.find('#_post_img').val().trim().length == 0 ) {
			...
			if( jQuery('.um-post-wall-modal-footer').length ){
				 jQuery('.um-post-wall-modal-footer .um-wall-msgs').html('<div class="wall-loader" />');
			}
			...
			jQuery.ajax({
				...
				success: function(data) {
					...
					if ( this_form.find('#_post_id').val() == 0 ) {
						...
						if( jQuery('.um-post-wall-modal-footer').length && data.message ){
							 jQuery('.um-post-wall-modal-footer .um-wall-msgs').html('<div class="wall-msgs">'+data.message+'</div>');
							 jQuery(".um-post-wall-modal-footer .um-wall-msgs .wall-msgs").delay(2000).fadeOut('slow');
							 setTimeout(function() {
								remove_Modal();
							}, 3000);
						}else if( jQuery('.um-post-wall-modal-footer').length ){
							jQuery('.um-post-wall-modal-footer .um-wall-msgs').remove('.wall-loader');
						}
